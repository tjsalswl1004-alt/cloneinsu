...
type GenId = 'gen1' | 'gen2a' | 'gen2b' | 'gen3' | 'gen4' | 'genSick';
type HospitalGrade = '병·의원' | '종합병원' | '상급종합병원';
...
interface GenConfig {
  limits: LimitOption[];
  outDed: Record<HospitalGrade, number>;
  rxDed: number;
  covRate: number;
  uncovRate: number;
  showRx: boolean;
...
const GEN_CONFIG: Record<GenId, GenConfig> = {
  gen1: {
    limits: [
...
const HOSPITAL_GRADES: HospitalGrade[] = ['병·의원', '종합병원', '상급종합병원'];
...
function ResultRow({ label, value, primary }: { label: string; value: number; primary?: boolean }) {
  return (
    <div className="flex items-center justify-between py-1.5">
      <span className={`text-sm ${primary ? 'font-bold text-primary' : 'text-gray-500'}`}>{label}</span>
      <span className={`text-sm ${primary ? 'font-bold text-primary' : 'text-gray-700'}`}>{fmt(value)}</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <input
        ...
        placeholder="0"
        className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white text-gray-800 focus:outline-none focus:border-primary"
      />
    </div>
  );
}

function HospitalGradeToggle({ grade, setGrade }: { grade: HospitalGrade; setGrade: (g: HospitalGrade) => void }) {
  return (
    <div>
      ...
      <div className="flex rounded-xl border border-gray-200 overflow-hidden">
        {HOSPITAL_GRADES.map((g) => (
          <button
            key={g}
            onClick={() => setGrade(g)}
            className={`flex-1 py-2 text-xs font-semibold transition-colors ${grade === g ? 'bg-primary text-white' : 'text-gray-500 bg-white'}`}
          >
            {g}
          </button>
        ))}
...
function NonCovered3Section({
  nc3Inj, setNc3Inj, nc3Manual, setNc3Manual, nc3Mri, setNc3Mri, nc3Ded, nc3Result,
}: {
  nc3Inj: string; setNc3Inj: (v: string) => void;
  nc3Manual: string; setNc3Manual: (v: string) => void;
  nc3Mri: string; setNc3Mri: (v: string) => void;
  nc3Ded: number; nc3Result: Nc3Result;
}) {
  const items = [
  ...
  return (
    <div className="bg-white rounded-2xl p-4 space-y-3">
      <h2 className="font-bold text-gray-900 text-sm">비급여 3종</h2>
...
export default function Calculator() {
  ...
  const [type, setType] = useState<'통원' | '입원'>('통원');
  const [genId, setGenId] = useState<GenId>('gen2a');
  const [limitIdx, setLimitIdx] = useState(0);
  const [grade, setGrade] = useState<HospitalGrade>('병·의원');
  ...
  const nc3Result = useMemo(() => {
    const calc = (v: string) => Math.max(0, Math.floor(((parseInt(v) || 0) - cfg.nc3Ded) * cfg.nc3Rate));
  ...
  return (
    <div className="min-h-screen pb-36" style={{ backgroundColor: '#F7F8FC' }}>
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 relative flex items-center justify-center">
        <button onClick={() => router.back()} className="absolute left-4 text-gray-500 text-2xl font-light leading-none">‹</button>
        <h1 className="text-base font-bold text-gray-900">실비 계산기</h1>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="bg-white rounded-2xl p-3">
          <div className="flex gap-2">
            {(['통원', '입원'] as const).map((t) => (
              <button
                key={t}
                onClick={() => setType(t)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-2 transition-colors ${
                  type === t ? 'border-primary text-primary bg-blue-50' : 'border-gray-200 text-gray-400'
                }`}
              >
                {t}
              </button>
        ...
        <div className="bg-white rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-gray-900 text-sm">세대 선택</h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            {GENERATIONS.map((gen) => (
              ...
              >
                <p className={`text-xs font-bold leading-tight ${genId === gen.id ? 'text-primary' : 'text-gray-800'}`}>{gen.label}</p>
        ...
        {type === '통원' && (
          <>
            <div className="bg-white rounded-2xl p-4 space-y-3">
              <h2 className="font-bold text-gray-900 text-sm">통원 (외래)</h2>
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm appearance-none bg-white text-gray-800 pr-8 focus:outline-none focus:border-primary"
                  >
            ...
            {cfg.showRx && (
              <div className="bg-white rounded-2xl p-4 space-y-3">
                <h2 className="font-bold text-gray-900 text-sm">통원 (약제)</h2>
                <div className="grid grid-cols-2 gap-3">
        ...
        {type === '입원' && (
          <>
            <div className="bg-white rounded-2xl p-4 space-y-3">
              <h2 className="font-bold text-gray-900 text-sm">입원</h2>
              <div className="grid grid-cols-2 gap-3">
      ...
      {/* 하단 고정 — 예상 보험금 */}
      <div className="fixed bottom-16 left-0 right-0 max-w-lg mx-auto z-40 px-4 pb-4">
