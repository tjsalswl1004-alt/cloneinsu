...
import { insuranceCompanyService } from '@/lib/api';
...
export default function InsuranceSelect() {
  ...
  const [activeTab, setActiveTab] = useState<string>('손해보험');
  const [selected, setSelected] = useState<InsuranceCompany | null>(null);
  const [companiesByCategory, setCompaniesByCategory] = useState<Record<string, InsuranceCompany[]>>({});
  ...
  const handleConfirm = () => {
    if (!selected) return;
    router.push(`/claim/new?companyId=${selected.id}&companyName=${encodeURIComponent(selected.name)}`);
  };
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: '#F7F8FC' }}>
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 relative flex items-center justify-center">
        ...
        <h1 className="text-base font-bold text-gray-900">보험사 선택</h1>
      </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {companies.map((company) => (
            <button
              ...
              className={`relative bg-white rounded-xl p-3 flex flex-col items-center gap-2 border-2 transition-colors ${
            ...
            >
              ...
              <span className="text-xs text-gray-700 text-center leading-tight">{company.name}</span>
            </button>
          ))}
        </div>
      </div>
