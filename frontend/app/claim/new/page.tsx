...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-primary"
      {...props}
    />
  );
}
      ...
      {options.map((opt) => (
        <button
          ...
          className={`flex-1 py-2.5 text-sm font-semibold transition-colors ${
            value === opt ? 'bg-primary text-white' : 'bg-white text-gray-500'
          }`}
        >
          {opt}
        </button>
...
const PersonIcon = ({ size, color }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
  </svg>
);
const HospitalIcon = ({ size, color }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const PenIcon = ({ size, color }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>
  </svg>
);
const PaperclipIcon = ({ size, color }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function ClaimNewContent() {
  ...
  useEffect(() => {
    if (!editId) return;
    claimService.getById(Number(editId)).then((data) => {
  ...
  const setVal = (field: keyof FormState) => (val: string) => setForm((prev) => ({ ...prev, [field]: val }));
  ...
  const handleDraft = () => {
    const action = editId
      ? claimService.update(Number(editId), buildPayload('DRAFT') as never)
      : claimService.create(buildPayload('DRAFT') as never);
    action.then(() => { alert('임시저장되었습니다.'); router.push('/claims'); }).catch(console.error);
  };

  const handleSubmit = () => {
    const action = editId
      ? claimService.update(Number(editId), buildPayload('SENT') as never)
      : claimService.create(buildPayload('SENT') as never);
    action.then(() => { alert('청구서가 제출되었습니다.'); router.push('/claims'); }).catch(console.error);
  };

  const headerCompanyName = form.insuranceCompanyName || '청구하기';
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: '#F7F8FC' }}>
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 relative flex items-center justify-center">
        ...
        <h1 className="text-base font-bold text-gray-900">{headerCompanyName}</h1>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        {/* 계약자 */}
        <SectionCard iconBg="#ECFDF5" iconColor="#10B981" icon={PersonIcon} title="계약자">
        ...
        <SectionCard iconBg="#FFF1F0" iconColor="#EF4444" icon={HospitalIcon} title="진료정보">
          ...
          <Field label="병원명">
            <Input placeholder="병원명 입력" value={form.hospitalName} onChange={set('hospitalName')} />
          </Field>
          <Field label="청구 금액">
            <Input type="number" placeholder="청구 금액 (원)" value={form.amount} onChange={set('amount')} />
          </Field>
        </SectionCard>
              ...
              <Field label="은행" required>
                <div className="relative">
                  <select
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-primary appearance-none bg-white"
                  >
                    <option value="">은행 선택</option>
                    {['KB국민은행','신한은행','우리은행','하나은행','농협은행','IBK기업은행','SC제일은행','씨티은행','카카오뱅크','케이뱅크','토스뱅크'].map((b) => (
          ...
          {form.signMethod === '대면 서명' && (
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm text-gray-700">서명란</span>
                <button type="button" onClick={clearCanvas} className="text-xs text-gray-400">
                  다시 서명
                </button>
              </div>
              <canvas
        ...
        <SectionCard iconBg="#F0FDFA" iconColor="#14B8A6" icon={PaperclipIcon} title="첨부서류" badge={files.length > 0 ? `${files.length}개` : null}>
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-1">
            <p className="text-xs font-bold text-gray-700 mb-2">필수 제출 서류</p>
