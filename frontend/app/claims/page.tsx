...
const TABS: { label: string; value: ClaimStatus | null; dotColor: string }[] = [
...
function formatAccidentDate(dateStr: string) {
  if (!dateStr) return '';
...
function formatIdFront(idFront?: string) {
...
function ClaimDetailCard({ claim, onDelete }: ClaimDetailCardProps) {
    ...
    <div className="bg-white rounded-2xl p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm text-gray-900">{companyName}</span>
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-2">
        ...
        {status === 'DRAFT' && (
          <button onClick={() => router.push(`/claim/new?id=${id}`)} className="flex items-center gap-1 text-xs text-orange-400 font-semibold">
...
function CustomerGroupCard({ patientName, phone, idFront, claims }: CustomerGroupCardProps) {
  return (
    <div className="bg-white rounded-2xl p-4 space-y-2">
      {/* 고객 헤더 */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="font-bold text-sm text-gray-900">{patientName}</span>
        ...
        {claims.map((claim) => {
          ...
          return (
            <div key={claim.id} className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400 whitespace-nowrap">{timeAgo(claim.createdAt)}</span>
...
export default function Claims() {
  ...
  const [activeTab, setActiveTab] = useState<ClaimStatus | null>(null);
  const [viewMode, setViewMode] = useState<'전체' | '고객별'>('전체');

  useEffect(() => {
    claimService.getAll().then(setClaims).catch(console.error);
  }, []);

  const countByStatus = (status: ClaimStatus | null) =>
    status ? claims.filter(c => c.status === status).length : claims.length;
